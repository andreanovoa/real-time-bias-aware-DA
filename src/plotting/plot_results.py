...
def plot_DA_window(t_obs, ax=None, twin=False, ens=None):
    ...
    if ens is not None:
        # new Estimator names first, legacy Ensemble names as fallback
        for names, cl, ll in zip([('start_bias', 'num_DA_blind'), ('start_param', 'num_SE_only')],
                                 ['darkblue', 'darkviolet'], ['BE', 'PE']):
            idx = next((getattr(ens, n) for n in names if hasattr(ens, n)), 0)
            if idx > 0:
