...
# This needs to br refradtored to the new repo
def run_plotting_annular_results(**kwargs):
    raise NotImplementedError('To be redefined after code restructuring')
...
#             out = load_from_pickle_file(results_dir + output_filename)
#             assert out
#             truth, results, bias, blank_ensemble = out
...
def plot_annular_model(forecast_params=None, animate=False, anim_name=None):
    ...
    if anim_name is None:
        anim_name = f'{datetime.date.today()}_Annulat_mov_mix_epsilon.gif'
    ...
    if not animate:
        ...
        def update(frame):
            ...
            ax2.set_title(f'Acoustic Pressure $t$ = {t_gif[frame]:.3f}')  # , fontsize='small')#, labelpad=50)

        plt.colorbar(polar_mesh, label='Pressure', shrink=0.75)
        anim = FuncAnimation(fig1, update, frames=len(t_gif)) # type: ignore
