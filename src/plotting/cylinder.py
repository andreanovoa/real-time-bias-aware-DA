...
from romda.plotting import *

path = os.path.dirname(__file__)
sys.path.append(os.path.abspath(os.path.join(path, '..')))


from romda.utils import interpolate, load_from_pickle_file, save_figs_to_pdf, save_to_pickle_file
...
def plot_timeseries(filter_ens, truth,
    ...
    _y_lims, x_lims = [min_y - margin, max_y + margin], [[t_hist[0], max_time], [t_obs[-1] - filter_ens.t_CR, max_time]]
        ...
        for ni, ns in enumerate(Ns_loop):
            ...
            for mi, m in enumerate(m_loop):
                ...
                for row_i, (param, param0) in enumerate(zip(alpha, alpha0)):
                    ...
                    if mi == 0:
                        ax.errorbar(-0.5,  np.mean(param0) , 2*np.std(param0), marker=markers[row_i], alpha=0.2, c= 'k', ms=4, capsize=2)
