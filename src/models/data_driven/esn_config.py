...
try:
    ESN_STORE = Path(get_project_root()) / 'results' / 'esn_configs'
except FileNotFoundError:  # imported from outside a project tree
    ESN_STORE = Path.cwd() / 'results' / 'esn_configs'
...
TRAINED_KEYS = ['norm', 'shift',
                ...
                'param_names', 'param_values', 'param_labels', 'input_parameters']
...
class ESNConfig:
    ...
    param_names: tuple = ()
    param_values: Optional[tuple] = None
    param_labels: Optional[tuple] = None
    input_parameters: Optional[np.ndarray] = None
        ...
        **override_kwargs
        ...
        if not retrain:
            ...
            for key in ('param_names', 'param_values', 'param_labels', 'input_parameters'):
                v = config.get(key)
                if v is None or (np.ndim(v) == 0 and not np.any(v)) or \
                        (np.ndim(v) > 0 and len(v) == 0):
                    config.pop(key, None)
...
def save_esn_model_to_config(esn_model, save_dir: Union[str, Path] =  ESN_STORE, name: Optional[str] = None):
...
def load_esn_model_from_config(q: Optional[str]=None,
                               ...
                               load_dir: Union[str, Path] = ESN_STORE):
...
def find_matching_config(search_dir: Union[str, Path], query_hash: str) -> Optional[Path]:
    """Search for a saved config directory matching `query_hash`.

    Parameters
    ----------
    search_dir : str or Path
        Directory to search for configs.
    query_hash : str
        Hash string of the desired configuration.

    Returns
    -------
    Path or None
        Path to the matching config directory, or `None` if not found.
    """
...
def auto_load_or_create(config_dir: Union[str,Path] = ESN_STORE,
                        auto_save: bool = True,
