...
#         psi0 = self.get_current_state
#         if not self.ensemble:
#             psi = [forecast(y0=psi0[:, 0], fun=self.time_derivative, t=t, params={**self.alpha0, **args})]

#         else:
#             if not averaged:
#                 alpha = self.get_alpha()
#                 forecast_part = partial(forecast, fun=self.time_derivative, t=t)
#                 sol = [self.__pool.apply_async(forecast_part,
#                                                kwds={'y0': psi0[:, mi].T, 'params': {**args, **alpha[mi]}})
#                        for mi in range(self.m)]
#                 psi = [s.get() for s in sol]
...
#                 if alpha is None:
#                     alpha = self.get_alpha(psi_mean0)[0]
#                 psi_mean = forecast(y0=psi_mean0[:, 0], 
#                                             fun=self.time_derivative,
#                                             t=t,
#                                             params={**alpha, **args}
#                                             )
