...
class Model:
    ...
    attr_ens = dict(m=10, est_p=[], est_s=True, est_b=False,
                    bias=None, inflation=1.01,
                    std_psi=0.1, std_a=0.1, alpha_distr='normal',
    ...
    def updateHistory(self, psi, t):
        ...
        # _________________________ FUNCTIONS TO FORECAST MODEL IN TIME ____________________________ #
    ...
    def timeIntegrate(self, Nt=100, averaged=False):
        """
            ...
            (Uses Parallel computation if self is an ensemble with m > 1)
            Args:
        ...
        if not averaged:
            ...
            # # print('\n 1 Processes and Queues: ' + str(time.time() - time1) + ' s')
            ...
            # time1 = time.time()
            # with mp.Pool() as p:
            #     psi = p.map(partial(Model.forecast, fun=self.timeDerivative, t=t, params=self_dict),
            ...
            #                     self.psi[:, range(self.m)].T)
            # psi = list(psi)
            # # print('3 map: ' + str(time.time() - time1) + ' s')
            # # ---------------------------------------------------------------------------------------------------
            ...
            #                      self.psi[:, range(self.m)].T)
            # psi = list(psi)
            # # print('4 imap: ' + str(time.time() - time1) + ' s')
            # # ---------------------------------------------------------------------------------------------------
            ...
            #                    self.psi[:, range(self.m)].T)
            # print('5 global pool + map: ' + str(time.time() - time1) + ' s')
            # # ---------------------------------------------------------------------------------------------------
            ...
            #                    self.psi[:, range(self.m)].T)
            # psi = list(psi)
            # print('6 global pool + map: ' + str(time.time() - time1) + ' s')
            # ---------------------------------------------------------------------------------------------------
        else:
            psi_mean = np.mean(self.psi, 1)

            ii = -self.Na
            alpha = self.alpha0.copy()
            for param in self.est_p:
                alpha[param] = psi_mean[ii]
                ii += 1

            # std = np.std(self.psi)
            psi_std = (self.psi - np.expand_dims(psi_mean, 1)) / np.expand_dims(psi_mean, 1)
            # psi = [odeint(self.timeDerivative, mean, t_interp, (self_dict,))]
            psi_mean = Model.forecast(y0=psi_mean, fun=self.timeDerivative, t=t, params=self_dict, alpha=alpha)
            psi = []
